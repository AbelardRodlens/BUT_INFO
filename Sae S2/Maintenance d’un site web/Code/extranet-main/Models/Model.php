...
class Model
{
    ...
    public function getDashboardGestionnaire()
    {
        ...
        co.nom_composante, 
        m.nom_mission, 
        COALESCE(p.nom, 'Aucun') AS nom, 
        COALESCE(p.prenom, 'Aucun') AS prenom, 
        m.id_personne AS id_prestataire, 
        m.id_mission 
    FROM 
        mission m
    JOIN 
        composante co ON m.id_composante = co.id_composante 
    JOIN 
        client c ON co.id_client = c.id_client 
    LEFT JOIN 
        personne p ON m.id_personne = p.id_personne;");
        $req->execute();
    ...
    {
        $req = $this->bd->prepare('SELECT p.id_personne AS id, nom, prenom FROM PERSONNE p JOIN PRESTATAIRE pr ON p.id_personne =  pr.id_personne;');
        $req->execute();
        return $req->fetchall();
    }
    ...
    public function getAllInterlocuteurs() 	{     	
        $req = $this->bd->prepare('SELECT personne.id_personne AS id, nom, prenom, nom_composante 
        FROM dirige JOIN composante USING(id_composante) JOIN personne USING(id_personne);');     	
        $req->execute();     	
        return $req->fetchall(); 	}

    public function getAllClientPrestataire($id_pr)
    { 
        $req = $this->bd->prepare("SELECT id_client AS id, nom_client FROM prestataire JOIN mission USING(id_personne) JOIN composante USING(id_composante) JOIN client USING(id_client) WHERE id_personne=:id;");
    ...
    public function getInfosComposante($id) 	{     	
        $req = $this->bd->prepare('SELECT id_composante, nom_composante, nom_client, numero, nomVoie, cp, ville, libelle    	FROM CLIENT JOIN COMPOSANTE using(id_client) JOIN ADRESSE USING(id_adresse) JOIN LOCALITE USING(id_localite) JOIN TYPEVOIE USING(id) WHERE id_composante = :id');     	
        $req->bindValue(':id', $id, PDO::PARAM_INT);     	
        $req->execute();     	
        return $req->fetchall()[0]; 	
    }
    ...
    {
        $req = $this->bd->prepare('SELECT DISTINCT p.id_personne, p.nom, p.prenom
FROM PERSONNE p
JOIN PRESTATAIRE pr ON p.id_personne = pr.id_personne
JOIN MISSION m ON p.id_personne = m.id_personne
WHERE m.id_composante = :id');
    ...
    public function getComponentsWithPrestataireMission($id_prestataire, $id_client)
    {
        $req = $this->bd->prepare('SELECT id_personne,COUNT(id_mission) AS "nb_mission", id_composante, nom_composante,id_client FROM composante JOIN mission USING(id_composante) WHERE id_personne=:id_prestataire AND id_client=:id_client GROUP BY id_personne,id_composante, nom_composante,id_client HAVING COUNT(id_mission) > 0;');
    ...
    public function getBdlComposante($id_composante) 	{     	
        $req = $this->bd->prepare('SELECT DISTINCT p.id_personne, id_bdl, nom, prenom, mois     	FROM PERSONNE p JOIN PRESTATAIRE pr USING(id_personne)  JOIN MISSION m USING(id_personne)  JOIN BON_DE_LIVRAISON b USING(id_mission)     	WHERE id_composante = :id');      	
    ...
    {
        $req = $this->bd->prepare("INSERT INTO ADRESSE(numero, nomvoie, id, id_localite) SELECT :num, :nomVoie, (SELECT id FROM TypeVoie WHERE libelle = :libelleVoie), (SELECT id_localite FROM localite WHERE cp = :cp)");
     ...
     * @return bool
     */
    public function addMission($nom, $date, $nom_compo, $nom_client)
    {
        $req = $this->bd->prepare("INSERT INTO MISSION (nom_mission, date_debut, id_composante) SELECT :nom, :date, (SELECT id_composante FROM COMPOSANTE JOIN CLIENT USING(id_client) WHERE LOWER(nom_client) = LOWER(:nom_client) and LOWER(nom_composante) = LOWER(:nom_composante))");
        $req->bindValue(':nom', $nom);
     ...
     * @return bool|void
     */
    public function addBdlInMission($nom_mission, $nom_composante, $annee,$mois, $id_prestataire)
    {
        try {
            $req = $this->bd->prepare("INSERT INTO BON_DE_LIVRAISON (mois, annee, id_mission, id_prestataire)
SELECT :mois, :annee, 
       (SELECT id_mission 
        FROM MISSION 
        JOIN COMPOSANTE USING(id_composante) 
        WHERE LOWER(nom_mission) = LOWER(:mission) 
          AND LOWER(nom_composante) = LOWER(:composante)
       ), 
       :id_prestataire;
");
    ...
    {
        $req = $this->bd->prepare("INSERT INTO MISSION (id_personne, nom_mission, id_composante) 
            SELECT 
                (SELECT p.id_personne FROM PERSONNE p WHERE p.email = :email),
                :nom_mission,
                :id_composante");
        $req->bindValue(':email', $email, PDO::PARAM_STR);
    ...
    {
        $req = $this->bd->prepare("SELECT id_bdl, mois, nom_mission 
FROM bon_de_livraison 
JOIN mission ON bon_de_livraison.id_mission = mission.id_mission 
JOIN prestataire ON mission.id_personne = prestataire.id_personne 
WHERE prestataire.id_personne = :id
");
        $req->bindValue(':id', $id_pr, PDO::PARAM_INT);
        $req->execute();
        return $req->fetchall();
    ...
    {
        $req = $this->bd->prepare("UPDATE ADRESSE
        SET numero = :num
        FROM COMPOSANTE
        WHERE ADRESSE.id_adresse = COMPOSANTE.id_adresse
        AND COMPOSANTE.id_composante = :id");
        $req->bindValue(':id', $id);
    ...
    {
        $req = $this->bd->prepare("UPDATE ADRESSE SET id  = (SELECT id FROM TYPEVOIE WHERE LOWER(libelle) = LOWER(:libelle))
    ...
    {
        $req = $this->bd->prepare('SELECT 
    cl.nom_client, comp.nom_composante, m.nom_mission, p.nom, p.prenom, m.id_mission, bdl.id_bdl, ed.id_personne AS id_prestataire
FROM 
    client cl
JOIN 
    composante comp ON cl.id_client = comp.id_client
JOIN 
    mission m ON comp.id_composante = m.id_composante
JOIN 
    estDans ed ON ed.id_composante = comp.id_composante
JOIN 
    PERSONNE p ON ed.id_personne = p.id_personne
JOIN 
    BON_DE_LIVRAISON bdl ON bdl.id_personne = ed.id_personne AND bdl.id_mission = m.id_mission
WHERE ed.id_personne = :id');
    ...
    {
        $req = $this->bd->prepare('SELECT nom_client, nom_composante, nom_mission, id_mission FROM client JOIN composante c USING(id_client) JOIN mission USING(id_composante) JOIN Prestataire USING(id_personne) WHERE id_personne=:id');
    ...
    {
        $req = $this->bd->prepare('SELECT DISTINCT p.nom, p.prenom, ta.id_personne AS id 
            FROM client c JOIN composante co ON c.id_client = co.id_client JOIN mission m ON co.id_composante = m.id_composante JOIN bon_de_livraison ta ON m.id_mission = ta.id_mission JOIN personne p ON ta.id_personne = p.id_personne JOIN estDans ed ON ed.id_composante = co.id_composante
            WHERE ed.id_personne = :id');
        $req->bindValue(':id', $id_co, PDO::PARAM_INT);
        $req->execute();
        return $req->fetchall();
    ...
    {
        $req = $this->bd->prepare("SELECT id_bdl, mois FROM BON_DE_LIVRAISON JOIN MISSION USING(id_mission) WHERE id_bdl = :id");
    ...
    {
        $req = $this->bd->prepare("SELECT id_bdl, nom_mission, annee,mois FROM BON_DE_LIVRAISON b JOIN MISSION m USING(id_mission) WHERE id_mission = :id_mission and m.id_personne = :id_prestataire");
    ...
    public function getClientContactDashboardData()
    {
        ...
        JOIN bon_de_livraison bdl ON m.id_mission = bdl.id_mission 
        WHERE bdl.id_personne = :id;');
    ...
    // public function getActiviteBdl($id_bdl){
    //     $req=$this->bd->prepare('SELECT commentaire,nb_demi_journee,date_bdl FROM Activite JOIN DEMI_JOUR USING (id_activite) WHERE id_bdl=:id_bdl');
    ...
    public function shearchIntoClientPrestataire($id_pr,$chaine,$chaine_copy)
    { // SELECT id_bdl, mois, nom_mission FROM bon_de_livraison JOIN prestataire ON id_personne = id_prestataire JOIN MISSION USING(id_mission) WHERE id_personne = :id
        $req = $this->bd->prepare("SELECT id_client AS id, nom_client FROM prestataire JOIN mission USING(id_personne) JOIN composante USING(id_composante) JOIN client USING(id_client) WHERE (id_personne=:id AND LOWER(nom_client) LIKE LOWER(:ch)) OR (id_personne=:id AND LOWER(nom_client) LIKE LOWER(:ch_cp))");
        $req->bindValue(':id', $id_pr, PDO::PARAM_INT);
        $req->bindValue(':ch', $chaine.'%',PDO::PARAM_STR);
        $req->bindValue(':ch_cp', '%'.$chaine_copy.'%',PDO::PARAM_STR);
        $req->execute();
        return $req->fetchall(PDO::FETCH_ASSOC);
    }

    public function shearchIntoComponentsPrestataire($id_prestataire, $id_client, $chaine, $chaine_copy)
    {
        $req = $this->bd->prepare('SELECT id_composante, nom_composante FROM composante JOIN mission m USING(id_composante) WHERE (m.id_personne=:id_prestataire AND id_client=:id_client AND LOWER(nom_composante) LIKE LOWER(:ch)) OR (m.id_personne=:id_prestataire AND id_client=:id_client AND LOWER(nom_composante) LIKE LOWER(:ch_cp))');
        $req->bindValue(':id_prestataire', $id_prestataire);
        $req->bindValue(':id_client', $id_client);
        $req->bindValue(':ch', $chaine.'%',PDO::PARAM_STR);
        $req->bindValue(':ch_cp', '%'.$chaine_copy.'%',PDO::PARAM_STR);
      ...
      // Récupérer les infos du bdl.
      public function getInfoBdl($id_bdl)
      {
          $req = $this->bd->prepare('SELECT annee, mois,commentaire, signatureInterlocuteur,signaturePrestataire FROM bon_de_livraison WHERE id_bdl=:id_bdl');
          $req->bindValue(':id_bdl', $id_bdl);
          $req->execute();
          return $req->fetch(PDO::FETCH_ASSOC);
      }
  
      // Récupérer le prestataire en charge du bdl.
      public function getBdlPrestataire($id_bdl)
      {
          $req = $this->bd->prepare('SELECT personne.id_personne, nom, prenom, email FROM personne JOIN Prestataire USING(id_personne) JOIN mission USING(id_personne) JOIN bon_de_livraison USING(id_mission) WHERE id_bdl=:id_bdl');
          $req->bindValue(':id_bdl', $id_bdl);
          $req->execute();
          return $req->fetch(PDO::FETCH_ASSOC);
      }
  
      // Récupérer les informations du client du bdl.
      public function getBdlClientInfos($id_bdl)
      {
          $req = $this->bd->prepare('SELECT id_client, nom_client, telephone_client, numero, nomVoie, libelle, cp FROM Bon_de_livraison JOIN mission USING (id_mission) JOIN composante USING(id_composante) JOIN client USING(id_client) JOIN adresse USING(id_adresse) JOIN localite USING(id_localite) JOIN typevoie USING(id) WHERE id_bdl=:id_bdl');
          $req->bindValue(':id_bdl', $id_bdl);
          $req->execute();
          return $req->fetch(PDO::FETCH_ASSOC);
      }
