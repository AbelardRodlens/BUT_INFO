...
function weekToDay($year,$month, $day){
    $weekStartDate = strtotime($year . '-' . $month .'-'.$day);
    $lastDayOfMonth = strtotime('last day of ' . $year . '-' . $month);
    $currentDay= $weekStartDate;
    $weekDays=[];
    while(date('N',$currentDay) <= 5 && $currentDay<=$lastDayOfMonth ){
        $weekDays[]=date('Y-m-d',$currentDay);
        $currentDay=strtotime('+1 day', $currentDay);
    }
