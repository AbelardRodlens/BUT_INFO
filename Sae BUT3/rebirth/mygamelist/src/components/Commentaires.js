...
const Commentaires = ({ commentaires, user, onAddComment, onReply, onLike }) => {
  ...
  const handleAddComment = async () => {
    ...
    // try {
    //   const response = await fetchWrapper("http://localhost:5001/game/add_comment", "POST", true, data);
  
    //   if (response.ok) {
    //     console.log("Commentaire ajouté avec succès.");
    //     const responseBody = await response.json();
    //     onAddComment(responseBody.data); 
    //     setNewComment(""); 
    //   } else {
    //     throw new Error("Problème rencontré lors de l'ajout du commentaire.");
    //   }
    // } catch (e) {
