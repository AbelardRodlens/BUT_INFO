...
const TopGames = () => {
  ...
  // Fonction pour mapper les champs du frontend vers les champs du backend
  const mapFieldToBackend = (field) => {
    const fieldMapping = {
      genre: "genres", // Corrige "genre" en "genres"
      theme: "themes",
      platform: "platforms",
      developer: "developers",
      publisher: "publishers",
    };
    return fieldMapping[field] || field; // Retourne la valeur mappée ou le champ original
  };
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        ...
        // Utiliser la fonction de mappage pour corriger le champ
        const mappedField = mapFieldToBackend(type);

        // Construire dynamiquement l'URL basée sur le champ corrigé
        const url = `http://localhost:5001/top_rated_by_field?field=${mappedField}&value=${value}`;
  ...
  return (
    <div className="top-games">
      ...
      {games.length > 0 ? (
        ...
        <p>
          Aucun jeu trouvé pour {getTypeLabel(type)} {value}.
        </p>
      )}
    </div>
  );
