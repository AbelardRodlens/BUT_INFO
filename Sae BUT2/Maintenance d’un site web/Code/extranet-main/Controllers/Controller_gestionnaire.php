...
class Controller_gestionnaire extends Controller
{
    ...
    public function action_dashboard()
    {
        ...
        if (isset($_SESSION['id'])) {
            ...
            $headerDashboard = ['Société', 'Composante', 'Nom Mission', 'Préstataire assigné','Status', 'Bon de livraison'];
    ...
    {
        return [['link' => '?controller=gestionnaire&action=clients', 'name' => 'Société', 'icon' => '<i class="fa-solid fa-landmark"></i>'],
            ['link' => '?controller=gestionnaire&action=composantes', 'name' => 'Composantes', 'icon' => '<i class="fa-solid fa-puzzle-piece"></i>'],
            ['link' => '?controller=gestionnaire&action=missions', 'name' => 'Missions', 'icon' => '<i class="fa-solid fa-list-check"></i>'],
            ['link' => '?controller=gestionnaire&action=prestataires', 'name' => 'Prestataires', 'icon' => '<i class="fa-solid fa-user"></i>'],
            ['link' => '?controller=gestionnaire&action=commerciaux', 'name' => 'Commerciaux', 'icon' => '<i class="fa-solid fa-euro-sign"></i>']];
    }
    ...
    public function action_ajout_client_form()
    {
        ...
        if (isset($_SESSION['id'])) {
            $bd = Model::getModel();
            $data = ['menu' => $this->action_get_navbar(), "person" => $bd->getAllInterlocuteurs()];
    ...
    {
        $bd = Model::getModel();
        if (isset($_POST['email-commercial']) && 
            ...
            header('Location: ?controller=gestionnaire&action=commerciaux');
            exit();
        }
            $data = ['menu' => $this->action_get_navbar()];
            $this->render('ajout_commercial', $data);
    ...
    public function action_ajout_prestataire()
    {
        ...
        if(isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['email-prestataire'])) {
            ...
            header('Location: ?controller=gestionnaire&action=prestataires');
            exit();
        }
            $data = ['menu' => $this->action_get_navbar()];
            $this->render('ajout_prestataire', $data);
    ...
    {
        $bd = Model::getModel();
       if (isset($_POST['email-commercial']) && isset($_GET['id_composante'])) {
            $bd->assignerCommercialByIdComposante(e($_POST['email-commercial']), e($_GET['id_composante']));
